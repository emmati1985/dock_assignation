				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("date") === oObject.date;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].date;
										} else {
											sCurrentId = aItemsNotInTheList[0].date;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("DockAssignation", {
											date : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("date"),
							title: oBindingContext.getProperty("dock")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "DockAssignation (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'DockAssignation (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("dock");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all DockAssignation to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "DockAssignation (" + iExpectedLength + ")"}),
									success : function () {
